\documentclass{scrartcl}
\usepackage{graphicx}
\usepackage[update,prepend]{epstopdf}
\usepackage{floatrow}
\usepackage{glossaries}
\usepackage{ifpdf}

%set path with figures to include inkscape latex+eps/pdf images from graphics dir
\graphicspath{{graphics/}}

\newglossaryentry{A}
{
  name=A,
  description={First letter}
}
\makeglossaries

\begin{document}
\glsaddall
\begin{figure}
\includegraphics[width=1cm]{graphics/gif.eps}
\caption{GIF image}
\end{figure}
\begin{figure}
\includegraphics[width=1cm]{graphics/jpg.eps}
\caption{JPG image}
\end{figure}
\begin{figure}
\includegraphics[width=1cm]{graphics/png.eps}
\caption{PNG image}
\end{figure}
\begin{figure}
\includegraphics[width=1cm]{graphics/svg.eps}
\caption{SVG image}
\end{figure}
%
\begin{figure}
\ifpdf
	\input{graphics/svg_tex.pdf_tex}
	\caption{SVG image (with combined Latex+PDF output)}
\else
	\input{graphics/svg_tex.eps_tex}	
	\caption{SVG image (with combined Latex+EPS output)}
\fi
\end{figure}
%
\begin{figure}
\includegraphics{graphics/ai.eps}
\caption{graphics/ai.eps}
\end{figure}
\begin{figure}
\includegraphics[width=5cm]{graphics/asy.eps}
\caption{Asymtote image}
\end{figure}
%
\begin{figure}
\ifpdf
	\includegraphics[width=5cm]{graphics/asy_pdf.pdf}
	\caption{Asymtote image directly converted to PDF}
\else
	\caption{WARNING! Cannot test Asymtote to PDF as latex was used to compile!}
\fi
\end{figure}
%
\begin{figure}
\includegraphics{graphics/dia.eps}
\caption{graphics/dia.eps}
\end{figure}
\begin{figure}
\includegraphics{graphics/fig.eps}
\caption{FIG image}
\end{figure}
\begin{figure}
\input{graphics/gnuplot.eps_tex}
\caption{Gnuplot image}
\end{figure}
\begin{figure}
\includegraphics{graphics/circuitmacros.eps}
\caption{Circuit\_macros image}
\end{figure}
\begin{figure}
\ifpdf
	\includegraphics{graphics/circuitmacros.pdf}
	\caption{Circuit\_macros image directly converted to PDF}
\else
	\caption{WARNING! Cannot test Circuit\_macros to PDF conversion as latex was used to compile!}
\fi
\end{figure}


\clearpage
\printglossaries
\end{document}
